...
#define SCAN_APP_PATH_FOLDER "apps_data/nrf24scan"
...
static void prepare_nrf24(bool fsend_packet)
{
  ...
  if(!fsend_packet) {
    ...
    if(what_to_do == 1) { // SNIFF
      ...
      nrf24_write_reg(nrf24_HANDLE, REG_FEATURE, *(rec + 2 + addr_size) >> 2 != 0x33 ? 4 : 0); // Enables the W_TX_PAYLOAD_NOACK command, Disable Payload with ACK, set Dynamic Payload
      if(*(rec + 1) & 0b100) { // ESB
      ...
      nrf24_write_reg(nrf24_HANDLE, REG_FEATURE, NRF_DPL ? 4 : 0); // Enables the W_TX_PAYLOAD_NOACK command, Disable Payload with ACK, set Dynamic Payload
...
//*/
  st = nrf24_rxpacket(nrf24_HANDLE, ptr + 2 + (what_to_do == 1 ? addrs_sniff.addr_len - 2 : 0), &packetsize, what_to_do == 1 ? true : !NRF_DPL);
  if(st & RX_DR) {
    st = (st >> 1) & 7; // pipe #
    if(what_to_do == 1) { // SNIFF
...
static void render_callback(Canvas* const canvas, void* ctx) {
  ...
  if(what_doing == 0) {
    ...
    if(NRF_ESB) {
      ...
      if(NRF_DPL) strcat_(screen_buf, " DPL");
    ...
    if(find_channel_period == 0) strcat_(screen_buf, "off"); else snprintf(screen_buf + strlen(screen_buf), sizeof(screen_buf), "%d s", find_channel_period);
    ...
    canvas_set_font(canvas, FontSecondary); // FIXME 5x7 font, 9 lines
    bool ch2 = false;
    screen_buf[0] = '\0';
    if(view_log_arr_x == 0) { strcat_(screen_buf, "  "); ch2 = true;
    } else {
        ...
        if(channel & 0x80) { // RAW packet: nn:>{.address..}-xxxxxxxx
          ...
          if(count > 0) {
            ...
            if(view_log_arr_x == 0) {
              ...
              for(int8_t j = 5 - adrsize; j > 0; j--) strcat_(screen_buf, "  ");
              strcat_(screen_buf, "-");
              pre += 5 * 2 + 1;
            } else {
    ...
    if(view_found >= 0) {
      ...
      for(uint8_t i = 0; i < 7; i++, idx++) {
        ...
        if(APP->found[idx].addr_size == 3) strcat_(screen_buf, "    "); else if(APP->found[idx].addr_size == 4) strcat_(screen_buf, "  ");
    ...
    if(view_log_decode_PCF || view_log_decode_CRC) {
      ...
      if(view_log_decode_PCF) strcat_(screen_buf, "ESB ");
      if(view_log_decode_CRC == 1) strcat_(screen_buf, "CRC1");
      else if(view_log_decode_CRC == 2) strcat_(screen_buf, "CRC2");
